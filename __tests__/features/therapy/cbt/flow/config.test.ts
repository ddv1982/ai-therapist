...
describe('CBT flow config', () => {
  it('persist functions should merge payloads correctly', () => {
    ...
    const thoughts = CBT_STEP_CONFIG.thoughts.persist(baseContext, [{ thought: 'x', credibility: 5 }] as any);
    expect(thoughts.thoughts?.[0].thought).toBe('x');

    const coreBelief = CBT_STEP_CONFIG['core-belief'].persist(baseContext, { coreBeliefText: 'bad', coreBeliefCredibility: 6 } as any);
    expect(coreBelief.coreBelief?.coreBeliefText).toBe('bad');

    const challenge = CBT_STEP_CONFIG['challenge-questions'].persist(baseContext, [{ question: 'q', answer: 'a' }] as any);
    expect((challenge.challengeQuestions as Array<{ answer: string }> | undefined)?.[0]?.answer).toBe('a');

    const rational = CBT_STEP_CONFIG['rational-thoughts'].persist(baseContext, { rationalThoughts: [{ thought: 'r', confidence: 7 }] } as any);
    expect(rational.rationalThoughts?.rationalThoughts?.[0].confidence).toBe(7);

    const schema = CBT_STEP_CONFIG['schema-modes'].persist(baseContext, { selectedModes: [{ id: '1', name: 'Mode', selected: true }] } as any);
    ...
    expect(actions.actionPlan).toMatchObject({ newBehaviors: 'new', originalThoughtCredibility: 4 });

    const final = CBT_STEP_CONFIG['final-emotions'].persist({ ...baseContext, actionPlan: undefined }, { fear: 2 } as any);
