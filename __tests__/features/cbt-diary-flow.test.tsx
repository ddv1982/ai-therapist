...
const flushTwo = async () => { await flush(); await flush(); };
...
function TestHarness() {
  ...
  return (
    <div>
      ...
      <button onClick={() => handleCBTSituationComplete({ situation: 'x', date: '2025-01-01' } as any)} data-testid="complete-situation" />
      <button onClick={() => handleCBTEmotionComplete({ fear: 1, anger: 0, sadness: 0, joy: 0, anxiety: 0, shame: 0, guilt: 0 } as any)} data-testid="complete-emotions" />
    </div>
  );
}
...
describe('useCbtDiaryFlow', () => {
  ...
  it('inserts next step component deterministically after completing Situation', async () => {
    ...
    await act(async () => { await flushTwo(); });
