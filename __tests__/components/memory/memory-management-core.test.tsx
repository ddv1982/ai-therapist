...
jest.mock('@/features/therapy/memory/session-report-detail-modal', () => ({
  SessionReportDetailModal: ({ open, onOpenChange, reportInfo, currentSessionId }: any) =>
    ...
    }, `Session detail for: ${reportInfo?.sessionTitle}`) : null,
}));

// Mock lucide-react icons
jest.mock('lucide-react', () => ({
  Trash2: (props: any) => React.createElement('svg', { ...props, 'data-testid': 'trash2-icon' }),
  AlertTriangle: (props: any) => React.createElement('svg', { ...props, 'data-testid': 'alert-triangle-icon' }),
  Calendar: (props: any) => React.createElement('svg', { ...props, 'data-testid': 'calendar-icon' }),
  FileText: (props: any) => React.createElement('svg', { ...props, 'data-testid': 'file-text-icon' }),
  CheckCircle: (props: any) => React.createElement('svg', { ...props, 'data-testid': 'check-circle-icon' }),
  ...
  RefreshCw: (props: any) => React.createElement('svg', { ...props, 'data-testid': 'refresh-cw-icon' }),
...
describe('MemoryManagementModal Core', () => {
  ...
  it('should show no memory state when no data exists', async () => {
    ...
    expect(screen.getByText('Manage your therapeutic session memory to control AI context and insights.')).toBeInTheDocument();
  });
