...
describe('RateLimiter', () => {
  ...
  describe('Exempt IP Handling', () => {
    ...
    it('should allow private network ranges', () => {
      const privateIPs = ['192.168.1.1', '10.0.0.1', '172.16.0.1', '172.17.0.1', '172.31.255.255'];
  ...
  describe('Suspicious Activity Detection', () => {
    it('should detect suspicious activity', () => {
      ...
      const foundSuspicious = suspicious.find(
        (entry: { fingerprint: string; ip: string; attempts: number; lastAttempt: number }) =>
    ...
    it('should not include non-suspicious IPs', () => {
      ...
      const foundIP = suspicious.find(
        (entry: { fingerprint: string }) => entry.fingerprint === normalFingerprint
      );
