...
describe('request-utils', () => {
  ...
  it('getClientIPFromRequest prefers x-forwarded-for then x-real-ip then request.ip', () => {
    // forwarded
    const req1 = {
      headers: new Headers({ 'x-forwarded-for': '100.100.100.100, 200.200.200.200' }),
    } as unknown as NextRequest;
    expect(getClientIPFromRequest(req1)).toBe('100.100.100.100');
    // real ip
    const req2 = {
      headers: new Headers({ 'x-real-ip': '123.123.123.123' }),
    } as unknown as NextRequest;
    expect(getClientIPFromRequest(req2)).toBe('123.123.123.123');
    // ip field fallback
    const req3 = { headers: new Headers(), ip: '8.8.8.8' } as unknown as NextRequest & {
      ip: string;
    };
