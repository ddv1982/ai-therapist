...
jest.mock('@/lib/auth/user-session', () => ({ getSingleUserInfo: jest.fn(() => ({ userId: 'u1' })) }));
...
describe('withApiRoute', () => {
  ...
  it('enforces auth when enabled', async () => {
    ...
    const res = await wrapped(new Request('http://test') as any, { params: Promise.resolve({}) } as any);
  ...
  it('rate limits when bucket denies', async () => {
    ...
    jest.isolateModules(() => {
      ...
      wrapped = api.withRateLimitUnauthenticated(async (_req: any, _ctx: any) => NextResponse.json({ success: true }) as any, { bucket: 'api' });
    });
    const res = await wrapped(new Request('http://test') as any, { params: Promise.resolve({}) } as any);
