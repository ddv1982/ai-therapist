...
describe('Crisis Detection System', () => {
  describe('detectCrisisContent', () => {
    ...
    test('should detect multiple categories', () => {
      const result = detectCrisisContent('I want to kill myself and I keep cutting to cope with the flashbacks');
    ...
    test('should provide appropriate recommended actions for crisis level', () => {
      ...
      expect(result.recommendedActions).toContain('Seek immediate professional help or contact emergency services');
      expect(result.recommendedActions).toContain('Reach out to a trusted friend, family member, or mental health professional');
      expect(result.recommendedActions).toContain('Remove any means of self-harm from your immediate environment');
    });

    test('should provide different actions for medium risk', () => {
      const result = detectCrisisContent('I feel very depressed and hopeless');
      
      expect(result.recommendedActions).toContain('Schedule an appointment with a mental health professional');
      expect(result.recommendedActions).toContain('Practice self-care and stress management techniques');
    });
  });
  ...
  describe('Edge Cases and Robustness', () => {
    ...
    test('should handle text with newlines and tabs', () => {
      const result = detectCrisisContent('I feel really bad\n\nI want to\tkill myself\n\nI cannot cope');
