...
describe('monitoring/performance-metrics', () => {
  ...
  it('records metrics and triggers threshold branches', () => {
    ...
    recordMetric({ type: MetricType.API_LATENCY, endpoint: '/api/a', duration: 100, timestamp: new Date() });
    // Warning branch
    recordMetric({ type: MetricType.API_LATENCY, endpoint: '/api/a', duration: 700, timestamp: new Date() });
    // Critical branch
    recordMetric({ type: MetricType.API_LATENCY, endpoint: '/api/a', duration: 1500, timestamp: new Date() });

    // DB warning and critical
    recordMetric({ type: MetricType.DATABASE_QUERY, operation: 'read', duration: 300, timestamp: new Date() });
    recordMetric({ type: MetricType.DATABASE_QUERY, operation: 'read', duration: 800, timestamp: new Date() });

    // Streaming warning and critical
    recordMetric({ type: MetricType.STREAMING_RESPONSE, endpoint: '/api/stream', duration: 3000, timestamp: new Date() });
    recordMetric({ type: MetricType.STREAMING_RESPONSE, endpoint: '/api/stream', duration: 6000, timestamp: new Date() });
  ...
  it('trackApiLatency wraps handler and records success and error', async () => {
    ...
    const errHandler = async () => { throw new Error('boom'); };
  ...
  it('trackDatabaseQuery measures both success and error paths', async () => {
    ...
    expect(res).toBe(42);
    await expect(trackDatabaseQuery('op', async () => { throw new Error('db'); })).rejects.toThrow('db');
  });

  it('provides metrics since N minutes and aggregated stats', () => {
    // Older than window
    recordMetric({ type: MetricType.API_LATENCY, endpoint: '/api/a', duration: 10, timestamp: new Date(Date.now() - 10 * 60 * 1000) });
    // Recent metrics
    for (let i = 0; i < 5; i++) {
      recordMetric({ type: MetricType.API_LATENCY, endpoint: '/api/a', duration: 100 + i, timestamp: new Date(), error: i % 2 === 0 });
      recordMetric({ type: MetricType.API_LATENCY, endpoint: '/api/b', duration: 200 + i, timestamp: new Date(), error: false });
    }
  ...
  it('computes system health and exports snapshot', () => {
    recordMetric({ type: MetricType.API_LATENCY, endpoint: '/api/a', duration: 800, timestamp: new Date() });
    recordMetric({ type: MetricType.API_LATENCY, endpoint: '/api/a', duration: 1200, timestamp: new Date() });
