...
describe('errors utils', () => {
  ...
  describe('specialized error handlers', () => {
    it('handleMessageError sets correct context', () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('system/high'), expect.any(Object));
    });
    ...
    it('handleAIError sets context for unavailable model', () => {
      handleAIError(new Error('model not available'));
       expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('external_api/medium'), expect.any(Object));
    });
  });
  ...
  describe('client side error utilities', () => {
    ...
    it('withErrorHandling catches and reports', async () => {
      ...
      const result = await withErrorHandling(async () => { throw new Error('oops'); }, {
        operationName: 'test',
        onError
      });
  ...
  describe('enhancedErrorHandlers', () => {
      it('handleDatabaseError delegates correctly', () => {
         ...
         expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('database/high'), expect.any(Object));
      });
  });
