...
describe('sessionsApi transforms', () => {
  ...
  it('createSession accepts wrapped and plain objects and throws otherwise', () => {
    const wrapped = transformCreateSessionResponse({ success: true, data: { id: 'x', title: 't', createdAt: '', updatedAt: '', messageCount: 0 } } as any);
    expect(wrapped.id).toBe('x');
    const plain = transformCreateSessionResponse({ id: 'y', title: 't2', createdAt: '', updatedAt: '', messageCount: 1 } as any);
  ...
  it('deleteSession accepts various success shapes', () => {
    ...
    expect(transformDeleteSessionResponse({ success: true, data: { success: true } } as any)).toEqual({ success: true });
  });

  it('getCurrentSession returns null for missing, supports wrapped and legacy shapes', () => {
    expect(transformGetCurrentSessionResponse({ success: true, data: { currentSession: { id: 's1' } } } as any)).toEqual({ id: 's1' });
    expect(transformGetCurrentSessionResponse({ currentSession: { id: 's2' } } as any)).toEqual({ id: 's2' });
    expect(transformGetCurrentSessionResponse({ success: true, data: { currentSession: null } } as any)).toBeNull();
