...
// ============================================================================
// SENSITIVE DATA KEYS AND PATTERNS
// ============================================================================
...
class Logger {
  ...
  private filterSensitiveData(obj: unknown, depth = 0): unknown {
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (
        SENSITIVE_THERAPEUTIC_KEYS.has(key) ||
  ...
  private formatLog(entry: LogEntry): string {
    ...
    const logData = {
      ...
      level,
      message,
      ...(filteredContext && typeof filteredContext === 'object'
  ...
  secureDevLog(message: string, data?: unknown): void {
    if (this.isDevelopment) {
      this.debug(
        `DEV: ${message}`,
        data ? { debugData: this.filterSensitiveData(data) } : undefined
      );
    }
  }
}
...
export function createRequestLogger(
  ...
  return {
    ...
    ip:
      ...
      ('connection' in req ? req.connection?.remoteAddress : undefined),
    method: req.method,
    url: req.url,
  };
}
