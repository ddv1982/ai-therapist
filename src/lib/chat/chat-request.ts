...
type RawMsg =
  | { role?: string; content?: unknown; parts?: RawPart[]; id?: unknown }
  | null
  | undefined;
...
export function buildForwardedMessages(
  ...
  const isForwardable = (
    m: RawMsg
  ): m is { role: 'user' | 'assistant'; content?: unknown; parts?: RawPart[]; id?: unknown } =>
  ...
  return (rawMessages as RawMsg[]).filter(isForwardable).map((m) => ({
