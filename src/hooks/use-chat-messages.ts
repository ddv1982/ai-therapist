...
export function useChatMessages() {
          ...
          setMessages((prev) =>
            prev.map((msg) => {
              ...
              const next: Message = {
                ...
                timestamp: normalizedTimestamp,
                modelUsed:
                  typeof data.modelUsed === 'string' && data.modelUsed.length > 0
                    ? data.modelUsed
                    : msg.modelUsed,
        ...
        setMessages((prev) =>
          prev.map((msg) => {
            ...
            const updatedMetadata =
              (savedMessage.metadata as Record<string, unknown> | null) ??
              message.metadata ??
              metadata;
  ...
  const updateMessageMetadata = useCallback(
    async (
      ...
      const previousMetadata = cloneMetadata(
        target.metadata as Record<string, unknown> | undefined
      );
      const baseMetadata =
        mergeStrategy === 'replace' ? {} : previousMetadata ? { ...previousMetadata } : {};

      let nextMetadata: Record<string, unknown>;
      try {
        const sanitizedIncoming = JSON.parse(JSON.stringify(metadata ?? {})) as Record<
          string,
          unknown
        >;
