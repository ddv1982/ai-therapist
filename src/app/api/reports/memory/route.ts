 ...
 */
export const GET = withAuth<MemoryData | MemoryManageData>(async (request: NextRequest, context: AuthenticatedRequestContext) => {
  try {
    ...
    if (manage) {
      ...
      const data = await service.getMemoryManagement(clerkId, limit, excludeSessionId, includeFullContent);
 ...
 */
export const DELETE = withAuth<DeleteResponseData>(async (request: NextRequest, context: AuthenticatedRequestContext) => {
  try {
    // Get clerkId from authenticated context (guaranteed to exist via withAuth)
    const userInfo = context.userInfo as { userId: string; clerkId?: string };
    const clerkId = userInfo.clerkId || userInfo.userId;
