...
export function RealisticMoon({ phase, size = 200, className = '' }: RealisticMoonProps) {
  ...
  const litPath = useMemo(() => {
    const { fraction } = phase;
    const p = fraction; // 0..1
    
    // r is the x-radius of the terminator ellipse
    // We want to map p:
    // 0.0 (New) -> Lit is 0.
    // 0.25 (1st Q) -> Lit is Right Half.
    // 0.5 (Full) -> Lit is Full.
    // 0.75 (3rd Q) -> Lit is Left Half.
    
    // Determine direction
    const isWaxing = p <= 0.5;
    
    // Angle for the terminator
    // We want a value that goes from 1 (New) to 0 (Quarter) to -1 (Full) for the "darkness" on the sun-side?
    // Let's use cos(2 * PI * p).
    // p=0 -> cos=1. p=0.25 -> cos=0. p=0.5 -> cos=-1.
    const terminatorX = radius * -Math.cos(2 * Math.PI * p);
  ...
  return (
    ...
    >
      ...
      <motion.div
        className="absolute inset-0 rounded-full bg-gradient-to-br from-primary/15 via-accent/10 to-transparent blur-3xl"
        style={{
          transform: 'scale(1.3)',
        }}
      ...
      <motion.div
        className="absolute inset-0 rounded-full bg-gradient-to-br from-primary/20 via-therapy-info/15 to-transparent blur-2xl"
        style={{
      ...
      <motion.div
        className="absolute inset-0 rounded-full bg-gradient-to-br from-accent/25 via-primary/20 to-transparent blur-xl"
        animate={glowAnimation}
        transition={{
          duration: 4,
          repeat: Infinity,
      ...
      <motion.div
        className="absolute inset-0 rounded-full bg-gradient-to-br from-white/0 via-white/5 to-white/0 blur-md opacity-0 transition-opacity duration-500 group-hover:opacity-100"
        style={{
      ...
      >
        <defs>
          ...
          <radialGradient id="moon-sphere" cx="45%" cy="45%" r="55%">
            <stop offset="0%" stopColor="currentColor" className="text-slate-50 dark:text-slate-50" />
            <stop offset="40%" stopColor="currentColor" className="text-slate-100 dark:text-slate-100" />
            <stop offset="70%" stopColor="currentColor" className="text-slate-200 dark:text-slate-200" />
            <stop offset="90%" stopColor="currentColor" className="text-slate-300 dark:text-slate-400" />
            <stop offset="100%" stopColor="currentColor" className="text-slate-400 dark:text-slate-500" />
          </radialGradient>
          ...
          <radialGradient id="rim-light" cx="50%" cy="50%" r="50%">
            ...
            <stop offset="95%" stopColor="currentColor" className="text-white/40 dark:text-white/30" />
          ...
          <radialGradient id="inner-shadow" cx="50%" cy="50%" r="50%">
            ...
            <stop offset="100%" stopColor="currentColor" className="text-black/15 dark:text-black/25" />
          </radialGradient>
        </defs>
        ...
        <circle 
          ...
          cy="100" 
          r="100" 
          className="fill-slate-700 dark:fill-slate-900"
        />

        {/* 2. Lit Portion - Enhanced 3D sphere gradient */}
        <path
          d={litPath}
          fill="url(#moon-sphere)"
        />
